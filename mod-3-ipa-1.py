...
def vigenere_cipher(message, key):
    ...
    message_ord = [ord(i) for i in message]
    cipherword = ""
    for i in range(len(message_ord)):
        if message[i].isupper():
            num = (message_ord[i] + key_ord[i % key_len]) % 26
            cipherword += chr(num + 65)
        else:
            cipherword += message[i]
    return cipherword
...
def scytale_cipher(message, shift):
    ...
    for i in range(newlen):
        rownew = i // cols
        colnew = i % cols
        cipherword[colnew * shift + rownew] = new[i]
...
def scytale_decipher(message, shift):
    ...
    # Replace `pass` with your code. 
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    len(message) % (len(message)//shift) == 0
    lenmessage = len(message)
    cols = lenmessage // (len(message)//shift)
    cipherword = ['_'] * lenmessage
    for i in range(lenmessage):
        newrow = i // cols
        newcol = i % cols
        cipherword[newcol * (len(message)//shift) + newrow] = message[i]
    return "".join(cipherword)
